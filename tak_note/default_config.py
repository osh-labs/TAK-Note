"""
Default configuration for TAK-Note.

All keys prefixed with OTS_TAKNOTE_ are read from ~/ots/config.yml.
...
class Config:
    ...
    OTS_TAKNOTE_POLL_INTERVAL: int = 30
    ...
    OTS_TAKNOTE_NOTEFILE_FILTER: str = ""
    ...
    OTS_TAKNOTE_COT_TYPE: str = "a-f-G-U-C"
    ...
    OTS_TAKNOTE_COT_STALE_TIME: int = 300
    ...
    OTS_TAKNOTE_WEBHOOK_ENABLED: bool = False
    ...
    OTS_TAKNOTE_WEBHOOK_SECRET: str = ""
    ...
    def validate(config: dict) -> list[str]:
        ...
        if not config.get("OTS_TAKNOTE_ENABLED", False):
            # Plugin disabled — nothing more to validate
            return errors

        if not config.get("OTS_TAKNOTE_API_KEY", "").strip():
            errors.append(
                "OTS_TAKNOTE_API_KEY is required when "
                "OTS_TAKNOTE_ENABLED is true"
            )

        project_uid = config.get("OTS_TAKNOTE_PROJECT_UID", "").strip()
        if not project_uid:
            errors.append(
                "OTS_TAKNOTE_PROJECT_UID is required when "
                "OTS_TAKNOTE_ENABLED is true"
            )
        elif not (project_uid.startswith("app:") or project_uid.startswith("product:")):
            errors.append(
                "OTS_TAKNOTE_PROJECT_UID must start with 'app:' or 'product:'"
            )

        poll_interval = config.get("OTS_TAKNOTE_POLL_INTERVAL", 30)
        try:
            if int(poll_interval) < 10:
                errors.append(
                    "OTS_TAKNOTE_POLL_INTERVAL must be at least 10 seconds"
                )
        except (ValueError, TypeError):
            errors.append(
                "OTS_TAKNOTE_POLL_INTERVAL must be an integer"
            )

        stale = config.get("OTS_TAKNOTE_COT_STALE_TIME", 300)
        try:
            if int(stale) < 1:
                errors.append(
                    "OTS_TAKNOTE_COT_STALE_TIME must be >= 1 second"
                )
        except (ValueError, TypeError):
            errors.append(
                "OTS_TAKNOTE_COT_STALE_TIME must be an integer"
            )

        if config.get("OTS_TAKNOTE_WEBHOOK_ENABLED", False):
            if not config.get("OTS_TAKNOTE_WEBHOOK_SECRET", "").strip():
                errors.append(
                    "OTS_TAKNOTE_WEBHOOK_SECRET should be set when "
                    "OTS_TAKNOTE_WEBHOOK_ENABLED is true (strongly recommended)"
                )
